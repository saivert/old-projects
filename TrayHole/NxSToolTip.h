...
class CNxSToolTip
{
  public:
	//- ctor & dtor -
	/* Uses GetModuleHandle(NULL) for hinst.
	 * Tip: You can do "CNxSToolTip g_tooltip(0);" and call "SetOwnerWnd()" on it later on.
	 *
	 * Set the IsBalloonPopup parameter to true to make the instance a balloon popup only
	 * object. You will only use the ShowBalloon method on it then.
	 */
	CNxSToolTip(HWND hwndOwner, bool IsBalloonPopup=false);
	CNxSToolTip(HWND hwndOwner, HINSTANCE hinst, bool IsBalloonPopup=false);
    ~CNxSToolTip();
	...
	void NewToolRect(UINT uId, LPRECT newrect);

	void RelayEvent(LPMSG msg);

	void TrackActivate(UINT uId, bool fActive);
	void TrackPosition(POINT pt);


	void ShowBalloon(LPCTSTR lpszText, LPCTSTR lpszTitle, DWORD dwFlags, POINT pt);
  ...
  protected:
	...
	bool m_IsBalloonPopup;

  private:
	HWND m_old_g_hwndTT;
	HWND m_old_g_hwndDlg;
	HHOOK m_old_g_hhk;
